...
- **Galaxy contour visual enhancements** — Solved a critical "wrap-around" bug in `Horizon.plot_horizon` where galaxy contours drew horizontal lines across the entire map when crossing the 360-degree azimuth boundary. Additionally, galaxy contour opacities are now uniform across density levels for better visual balance, and the "Galaxy" trace is properly labeled in the plot's legend.
