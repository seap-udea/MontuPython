...
def rise_set_az(seba, site, horizon_el=0.0):
    """Rise and set azimuths at the observer epoch already set on *site*."""
    import ephem as pyephem
...
def compute_extremes(name):
    """Northernmost/southernmost rise and set azimuths over the search window."""
    body = make_body(name)
    rise_azs, set_azs = [], []
    circumpolar = neverup = False
...
def build_disk_figure(extremes, *, year, era, observer_name="Giza", lat=29.9792):
    """Polar orientation disk — same layout as MontuPython Desktop."""
    fig = go.Figure()
