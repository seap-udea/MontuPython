...
from montu_gui.utils.module_icons import module_brand_pixmap
...
def _module_descriptions_by_key() -> dict[str, str]:
    ...
    for entry in load_home_content().get("modules", []):
        key = entry.get("module_key") or icon_to_key.get(entry.get("icon", ""))
        if key:
...
class ModuleBrand(QFrame):
    ...
    def __init__(
        ...
        *,
        ...
        brand_pixmap = module_brand_pixmap(module_key)
        if brand_pixmap is not None:
            icon_lbl = QLabel()
            icon_lbl.setObjectName("module_brand_image")
            icon_lbl.setAlignment(Qt.AlignmentFlag.AlignCenter)
            icon_lbl.setPixmap(brand_pixmap)
            icon_lay.addWidget(icon_lbl)
        else:
            emoji = QLabel(_MODULE_ICONS.get(module_key, "•"))
